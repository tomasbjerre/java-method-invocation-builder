...
public class ElementHandler {
 ...
 public void handle(TypeElement classElement) throws IOException {
  ...
  writeResults(classElement, classModel);
 }
 ...
 private void writeResults(TypeElement classElement, ClassModel classModel) throws IOException {
  for (ClassMethod classMethod : classModel.getMethods()) {
   ...
   JavaFile javaFile = new CodeGenerator().generateJavaFile(classModel.getPackageName(), classModel.getClassName(),
     classModel.getClassName() + classSuffix, classMethod);
   try {
   ...
   this.messager.printMessage(Diagnostic.Kind.NOTE,//
     "Created: " + newJavaFileObject.toUri() + "\n" //
